...
def main(args):
    ...
    results_csv = os.path.join(args.output_csv if args.output_csv else "results.csv")
    with open(results_csv, "w", newline="") as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(["Rank", "ID", "Similarity", "Example_File"])
        ...
        for i, idx in enumerate(topk.indices):
            ...
            writer.writerow([i+1, match_id, f"{sim_score:.4f}", os.path.basename(match_files[0])])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_csv", type=str, default="results.csv", help="CSV file to store results")
